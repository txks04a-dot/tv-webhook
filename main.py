...
# Use persistent disk mount for NDJSON if available.
# Set env: DATA_DIR=/var/data (or your disk mount path)
# ============================================================
...
# Day boundary: align your "day" with NY session
...
_trade_events = []       # [{"id": int, "ts": float, "data": dict}, ...]
_next_event_id = 1
...
def atomic_write_ndjson(path: str, records) -> None:
...
def open_trades_for_symbol(symbol: str):
  ...
  if not os.path.exists(TRADES_PATH):
      return []
  return [
      t for t in ndjson_read_all(TRADES_PATH)
...
# Metrics
# -----------------------------
def compute_metrics(trades, last_n=None) -> dict:
...
# -----------------------------
# SSE Routes (MUST be above __main__)
# -----------------------------
@app.route("/stream", methods=["GET"])
def stream():
  """
  Server-Sent Events stream of trade_open events.
  Browser uses: new EventSource('/stream')
  """
  try:
  ...
  # use a mutable box to avoid nonlocal
  state = {"last_id": last_id, "hb": 0}

  def event_generator():
      while True:
          new_events = [e for e in _trade_events if e["id"] > state["last_id"]]
          for e in new_events:
              state["last_id"] = e["id"]
              yield f"id: {e['id']}\n"
...
def webhook():
  ...
  if event_type == "bar":
      ...
      # master bar payload: {"prices": {"EURUSD": 1.23, "USDJPY": 150.1, ...}}
